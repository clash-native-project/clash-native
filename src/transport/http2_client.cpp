...
class Http2ClientSession final : public io::ExchangeSession,
  ...
  public:
    ...
    exchange_streaming(io::StreamingExchangeRequest request,
        ...
        if (body_error) {
            channel.sender.send(core::fail(*body_error));
            return wrap_streaming(exchange_id, std::move(channel.receiver));
        }
        if (deadline <= std::chrono::steady_clock::now()) {
            channel.sender.send(core::fail(timeout_error()));
    ...
    open_tunnel(io::StreamUpgradeRequest request,
        ...
        if (request.authority.empty() || contains_uri_whitespace(request.authority) ||
            ...
            channel.sender.send(core::fail(core::Error{
                core::ErrorCode::configuration, "HTTP/2 tunnel request fields are invalid"}));
            return wrap_tunnel(exchange_id, std::move(channel.receiver));
        }
        ...
        if (request.mode == io::StreamUpgradeMode::upgrade && peer_connect_protocol_received_ &&
    ...
    std::size_t active_streams() const noexcept { return stream_pending_.size(); }
  ...
  private:
    struct Pending {
        ...
        std::vector<io::ExchangeField> request_trailers;
    ...
    resolve_content_length(const io::StreamingExchangeRequest &request,
    ...
    io::AnySender<typename Terminal::value_type> wrap(io::ExchangeSession::ExchangeId exchange_id,
        ...
        auto sender =
            ...
            stdexec::then([](std::optional<Terminal> terminal) -> typename Terminal::value_type {
                if (!terminal) {
                    throw core::Error{core::ErrorCode::cancelled, "HTTP/2 exchange was abandoned"};
                }
                if (!*terminal) {
    ...
    void handle_response_headers(ExchangeId exchange_id) {
        ...
        pending->response_body = std::make_shared<io::detail::QueuedExchangeBodyStream>(
