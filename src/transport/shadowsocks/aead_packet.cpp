...
encrypt_aead_datagram(std::string_view method, std::string_view password,
    ...
    if (!spec || spec.value().kind != transport::proxy::CipherKind::aead ||
    ...
    auto key = transport::proxy::derive_aead_subkey(method, password, salt);
    if (!key) {
        return core::fail(key.error());
    }
    const std::vector<std::uint8_t> nonce(spec.value().nonce_size, 0);
    auto ciphertext = transport::proxy::aead_encrypt(method, key.value(), nonce, plaintext);
    if (!ciphertext) {
...
core::Result<std::vector<std::uint8_t>> decrypt_aead_datagram(std::string_view method,
    ...
    const std::vector<std::uint8_t> nonce(spec.value().nonce_size, 0);
    return transport::proxy::aead_decrypt(method, key.value(), nonce,
                                          wire.subspan(spec.value().key_size));
}
